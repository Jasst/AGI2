...
class AdvancedBPETokenizer:
    ...
    def _init_bpe_tokenizer(self):
        ...
        # ✅ ДОБАВИТЬ специальные токены в словарь
    ...
    def encode(self, text: str, max_length: Optional[int] = None) -> List[int]:
        ...
        if TOKENIZERS_AVAILABLE and self.tokenizer:
            try:
                ...
                logger.warning(f"Tokenizer encode error: {e}, using fallback")
                # Fallback на простой сплит
                words = text.lower().split()
...
class RAGSystem:
    ...
    def __init__(self, embedding_dim: int = 768):
            ...
            self.enabled = CHROMADB_AVAILABLE  # ✅ Локальная копия
            ...
            try:
                ...
                self.enabled = False  # ✅ Меняем локальную переменную, не глобальную!
    ...
    def add_interaction(
        ...
        if not self.enabled or not self.collection:  # ✅ Используем self.enabled
            return
...
• Адаптивнее (Meta-Learning)</b>"""
    ...
    async def start_polling(self):
        ...
        # ✅ allowed_updates=[] предотвращает конфликт
