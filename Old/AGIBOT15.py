# coding: utf-8
"""
Cognitive_Agent_Pro_v9.5_SINGLE_MODEL.py
🧠 Когнитивный агент - работа с УЖЕ ЗАГРУЖЕННОЙ моделью
✅ Исправлена ошибка с несколькими моделями в LM Studio
✅ Улучшенная логика интернет-поиска
...
class EnhancedWebSearch:
    """Улучшенный веб-поиск с поддержкой различных источников"""
    ...
    async def search(self, query: str) -> Tuple[str, str]:
        """Поиск с кэшированием, возвращает (результаты, тип_запроса)"""
        ...
        if query_type == "currency":
            result = await self._search_currency()
        ...
        if not result:
            result = f"По запросу '{query}' информация не найдена в открытых источниках."

        # Сохраняем в кэш
        self.cache[cache_key] = (result, query_type)
    ...
    def _detect_query_type(self, query: str) -> str:
        ...
        currency_words = ['курс', 'доллар', 'евро', 'валют', 'рубл', 'юан', 'биткоин', 'крипто']
        weather_words = ['погода', 'температура', 'дождь', 'снег', 'солнце', 'ветер', 'град', 'жара', 'холод']
        news_words = ['новости', 'события', 'последние', 'сегодня', 'сейчас', 'актуальные']

        if any(word in query_lower for word in currency_words):
            return "currency"
        elif any(word in query_lower for word in weather_words):
            return "weather"
        elif any(word in query_lower for word in news_words):
            return "news"
        else:
            return "general"

    async def _search_currency(self) -> str:
    ...
    async def _search_weather(self, query: str) -> str:
        """Поиск погоды"""
        try:
            # Базовый ответ о погоде (можно подключить реальный API)
            cities = ["москв", "санкт-петербург", "новосибирск", "екатеринбург", "казан"]
            found_city = None
            ...
            city_name = "Москве" if not found_city else found_city.title()
            now = get_current_datetime()

            # Имитация данных погоды
            import random
            temp = random.randint(-10, 25)
            conditions = ["ясно", "облачно", "небольшой дождь", "снег", "пасмурно"]
            condition = random.choice(conditions)

            return (
                f"🌤️ Погода в {city_name} на {now['time']}:\n"
                f"📅 {now['full_date']}\n\n"
                f"🌡️ Температура: {temp}°C\n"
                f"☁️ Состояние: {condition}\n\n"
                f"ℹ️ Для точных данных используйте специализированные сервисы погоды."
            )
    ...
    async def _search_news(self, query: str) -> str:
        ...
        try:
            ...
            return (
                f"📰 Последние новости ({now['time']}):\n\n"
                f"1. Технологии: Развитие искусственного интеллекта продолжает ускоряться\n"
                f"2. Экономика: Рынки демонстрируют стабильность\n"
                f"3. Наука: Новые открытия в области квантовых вычислений\n\n"
                f"ℹ️ Для получения актуальных новостей используйте новостные агрегаторы."
            )
    ...
    async def _search_general(self, query: str) -> str:
        ...
        try:
            ...
            log_stage("⚠️ ПОИСК", f"Ошибка общего поиска: {e}", Colors.YELLOW)

        return f"По запросу '{query}' найдена общая информация. Для получения точных данных уточните запрос."
...
class LearningMemory:
    ...
    def learn_from_interaction(self, query: str, approach: str, confidence: float, used_search: bool):
        ...
        # Добавляем информацию о поиске в подход
        if used_search:
        ...
        if pattern_key in self.patterns:
            ...
            # Обновляем подход, если новый лучше
...
class ActiveModelInterface:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    ...
                    log_stage("⚠️ ОШИБКА", f"Не удалось получить модели: {e}", Colors.YELLOW)

                # Пробуем сделать тестовый запрос
                try:
                    payload = {
                        "model": "unknown",  # Временное значение
                    ...
                    async with session.post(
                        ...
                        if resp.status == 200:
                            ...
                            # Пробуем понять, какие модели доступны по ошибке
                            try:
                                error_data = await resp.json()
                                error_msg = error_data.get('error', {}).get('message', '')
                                if 'models' in error_msg:
                                    # Парсим имена моделей из ошибки
                                    import re
                                    models = re.findall(r"'([^']+)'", error_msg)
                                    if models:
                                        self.model_name = models[0]
                                        log_stage("✅ НАЙДЕНА", f"Модель: {self.model_name} (из ошибки)", Colors.GREEN)
                                        return True
                            except:
                                pass
                except Exception as e:
    ...
    async def generate(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: добавляем поле model в payload
                messages = []

                if system_prompt and system_prompt.strip():
                    messages.append({"role": "system", "content": system_prompt[:1000]})

                messages.append({"role": "user", "content": user_prompt[:2500]})

                payload = {
                    "model": self.model_name,  # Явно указываем модель
                    "messages": messages,
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        log_stage("✅ УСПЕХ", f"Ответ получен ({len(content)} chars, {confidence:.0%} уверенность)",
                                  Colors.GREEN)
                        ...
                        if resp.status in [400, 404]:
                            log_stage("🔧 ДЕБАГ", f"Payload: {json.dumps(payload, ensure_ascii=False)[:200]}...",
                                      Colors.YELLOW)
...
class CognitiveBrain:
    ...
    def __init__(self, model: ActiveModelInterface, memory: LearningMemory):
        self.model = model
        self.memory = memory
        self.search_engine = EnhancedWebSearch()
    ...
    async def think(self, query: str, context: str = "") -> CognitiveResponse:
        ...
        # ШАГ 1: Модель решает, нужен ли поиск
        needs_search, search_query = await self._decide_search_needs(query)
        search_results = ""
        search_query_used = ""

        if needs_search and search_query:
            search_query_used = search_query
            log_stage("🔍 ПОИСК", f"Модель запросила поиск: '{search_query}'", Colors.BLUE)
            search_results, search_type = await self.search_engine.search(search_query)

            if search_results:
                log_stage("✅ ПОИСК", f"Найдена информация ({search_type})", Colors.GREEN)
            else:
                log_stage("⚠️ ПОИСК", "Информация не найдена", Colors.YELLOW)

        # ШАГ 2: Формируем промпт с учетом поиска
        learned_approach = self.memory.get_learned_approach(query)

        system_prompt = f"""Ты — умный и полезный ассистент с доступом к интернету.
...
2. Используй предоставленную информацию из поиска
3. Если информации недостаточно, честно скажи об этом
4. Будь точным, информативным и полезным
5. Форматируй ответ для лучшей читаемости"""
    ...
    async def _decide_search_needs(self, query: str) -> Tuple[bool, str]:
        """Модель сама решает, нужен ли поиск и формирует запрос"""
        # Сначала проверяем очевидные случаи
        obvious_search = [
            'курс', 'доллар', 'евро', 'валют', 'цена', 'стоимость',
            'погода', 'температура', 'новости', 'сейчас', 'сегодня',
            'акции', 'биржа', 'крипто', 'биткоин'
        ]

        if any(word in query.lower() for word in obvious_search):
            # Для простых запросов возвращаем сам запрос
            return True, query

        # Для сложных случаев спрашиваем у модели
        decision_prompt = f"""Вопрос пользователя: "{query}"

Нужно ли искать информацию в интернете для ответа на этот вопрос?
Если ДА, сформулируй краткий поисковый запрос (3-7 слов).
Если НЕТ, просто напиши "НЕТ".

Примеры:
- "Какой сегодня курс доллара?" → ДА, "курс доллара ЦБ РФ сегодня"
- "Что такое искусственный интеллект?" → НЕТ
- "Какая погода в Москве?" → ДА, "погода в Москве сейчас"

Твой ответ:"""

        try:
            decision, _ = await self.model.generate(
                system_prompt="Ты определяешь, нужен ли поиск в интернете. Отвечай кратко.",
            ...
            if "ДА" in decision.upper() or ":" in decision:
                ...
                return True, search_query[:100]
            else:
                return False, ""

        except Exception as e:
            log_stage("⚠️ РЕШЕНИЕ", f"Ошибка определения поиска: {e}", Colors.YELLOW)
            return False, ""
...
class CognitiveDatabase:
    ...
    def save_conversation(self, user_id: int, query: str, response: CognitiveResponse):
        ...
        cursor.execute('''
            ...
            (user_id, timestamp, query, response, confidence, used_search, search_query, processing_time)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            user_id,
            time.time(),
...
class TelegramBot:
    ...
    async def _handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        welcome = (
            ...
            f"Я — **Когнитивный Агент v9.5** 🤖\n\n"
            f"✨ **Что я умею:**\n"
            f"• Отвечать на вопросы с помощью ИИ\n"
            f"• Автоматически искать информацию в интернете\n"
            f"• Запоминать контекст разговора\n"
            f"• Учиться на диалогах\n\n"
            f"🔍 **Поиск:** Я сам решаю, когда искать информацию!\n\n"
    ...
    async def _handle_search(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            search_engine = EnhancedWebSearch()
            ...
            log_stage("❌ ОШИБКА", f"Поиск: {e}", Colors.RED)
    ...
    async def _handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            error_msg = (
                ...
                "3. Ошибка в работе модели\n\n"
                "Попробуйте еще раз или проверьте работу LM Studio."
            )
    ...
    async def _handle_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "stats":
            ...
            help_text = (
                ...
                f"• Новости\n"
                f"• Актуальные данные\n\n"
                f"Я учусь на каждом диалоге и становлюсь умнее!"
            )
    ...
    def _is_simple_query(self, query: str) -> bool:
        ...
        if any(word in query_lower for word in ['привет', 'здравствуй', 'hello', 'hi', 'хай', 'добрый']):
            return True
    ...
    def _handle_simple_query(self, query: str) -> str:
        ...
        if re.search(r'какой день недели|какой день', query_lower):
...
async def main():
    ...
    print(f"🧠 КОГНИТИВНЫЙ АГЕНТ v9.5")
    ...
    try:
        ...
        print(f"  • Автопоиск: ВКЛЮЧЕН (модель решает сама)")
