# coding: utf-8
"""
AGI_Cognitive_Agent_LMStudio_v4.py — УЛУЧШЕННАЯ ВЕРСИЯ С ВЕБ-ПОИСКОМ И МЕТАКОГНИЦИЕЙ
...
✅ Сохранение 100% приватности для локальных запросов
✅ Адаптивное определение необходимости поиска
"""
import asyncio
import logging
import os
...
try:
    # На:
    try:
        from ddgs import AsyncDDGS  # Новый пакет

        DDGS = AsyncDDGS
    except ImportError:
        from duckduckgo_search import DDGS  # Старый пакет как fallback
    from bs4 import BeautifulSoup

    HAS_WEB_SEARCH = True
    print("✅ Модули веб-поиска загружены (duckduckgo-search, beautifulsoup4)")
except ImportError as e:
    HAS_WEB_SEARCH = False
    print(f"⚠️ Модули веб-поиска недоступны: {e}")
    print("📦 Установите: pip install duckduckgo-search beautifulsoup4 lxml")
    ...
    def get_lmstudio_config(cls) -> Dict[str, Any]:
        ...
        if not os.getenv("LM_STUDIO_CONFIGURED"):
            ...
            if Config.WEB_SEARCH_ENABLED:
                ...
                print("   • ⚠️ Веб-поиск: отключён (установите duckduckgo-search)")
            print("=" * 70 + "\n")
        return config
...
class WebSearchEngine:
    ...
    async def search(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = []
            search_results = self.ddgs.text(query, max_results=max_results, safesearch='moderate')
...
class UserCognitiveAgent:
    ...
    def normalize_search_query(query: str) -> str:
        ...
        # Словарь опечаток → правильные варианты
        replacements = {
            r'\bдолар\b': 'доллар',
            r'\bдолларов\b': 'долларов',
            r'\бодолар(е|у|ом)?\b': 'о доллар',
        ...
        # Удаляем двойные пробелы и лишние символы
    ...
    async def _needs_web_search(self, text: str, features: Dict) -> bool:
        ...
        if not Config.WEB_SEARCH_ENABLED:
            return False
        normalized = self.normalize_search_query(text)  # <-- ДОБАВИТЬ ЭТУ СТРОКУ
        text_lower = normalized.lower()  # <-- ИСПОЛЬЗОВАТЬ НОРМАЛИЗОВАННЫЙ ТЕКСТ
        ...
        current_keywords = ['сейчас', 'сегодня', 'последние', 'актуальн', 'новост', 'текущ', 'свежие']
    ...
    async def _perform_web_search(self, query: str) -> str:
        ...
        if not self.search_engine:
            return "⚠️ Веб-поиск недоступен. Установите модули duckduckgo-search и beautifulsoup4."
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        ...
        "• 🔍 Автоматический и ручной веб-поиск актуальной информации\n"
        "• 💡 Метакогнитивный мониторинг качества ответов\n"
        "• 🎯 Декомпозиция сложных целей на подзадачи\n"
...
async def decompose_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    agent = await context.application.bot_data['session_manager'].get_or_create_session(update.effective_user.id)
    await update.message.reply_chat_action("typing")
    await update.message.reply_text(f"🎯 Анализирую цель: {goal}...")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if handler:
        if callable(handler):
            # Передаём оригинальный update - команды теперь используют effective_message
...
async def main():
    ...
    print(
        f"   Веб-поиск: {'✅ Активирован' if Config.WEB_SEARCH_ENABLED else '❌ Отключён (установите duckduckgo-search)'}")
...
def run():
    print("AGI Cognitive Assistant v4 — Улучшенная когнитивная система с веб-поиском и метакогницией")
