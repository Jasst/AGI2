...
"""
AGI6.2_fixed.py
Исправленная когнитивная сеть:
- Все клетки на правильном устройстве (GPU/CPU)
- Теги [SOC]/[FCT] не токенизируются как слова
- Новые слова инициализируются как <UNK>
- Мышление фокусируется на релевантных типах
- Обучение на полных последовательностях с тегами
- Генерация учится повторять структуру Qwen
"""
import os
import re
import random
...
class CognitiveNetwork(nn.Module):
    ...
    def add_cell(self, cell_type: str, input_size: int, hidden_size: int, cell_subtype: str = "association") -> str:
        ...
        self.cells[cell_id].to(self.device)  # 🔥 Важно!
    ...
    def process_sequence(self, input_tokens: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, allowed_stages: Optional[List[str]] = None) -> torch.Tensor:
    ...
    def generate_sequence(self, input_tokens: torch.Tensor, allowed_stages: List[str], max_length: int = 30, temperature: float = 0.8) -> List[int]:
        ...
        with torch.no_grad():
            ...
            for t in range(combined.size(1)):
                token_emb = combined[:, t, :]
                _ = self._adaptive_thought_cycles(token_emb, allowed_stages, max_cycles=3)
            generated = []
            current = input_tokens[:, -1:].clone()
            for _ in range(max_length):
                emb = self.embedding(current).squeeze(1)
                emb = self.embed_proj(emb)
                logits = self._adaptive_thought_cycles(emb, allowed_stages, max_cycles=3)
    ...
    def expand_vocab(self, new_vocab_size: int):
        ...
        # Инициализация новых слов как <UNK>
        unk_idx = 2
    ...
    def load_knowledge(self, filepath: str):
        ...
        try:
            ...
            for config in cell_configs:
                ...
                self.cells[cid].to(self.device)  # 🔥 КРИТИЧЕСКИ ВАЖНО!
...
class BrainTeacher:
    ...
    def teach_brain(self, brain: CognitiveNetwork, user_input: str, vocab: Dict[str, int], ivocab: Dict[int, str],
        ...
        # ВАЖНО: обучаем на тегированной версии!
        ...
        brain.replay.add(input_seq, target_seq, meta={"qwen": tagged_qwen}, priority=1.0)
        ...
        sorted_pairs = sorted(zip(batch_inputs, batch_targets, [1.0] + is_weights, [None] + idxs), key=lambda x: len(x[0]), reverse=True)
...
def main():
    print("🧠 Запуск исправленного когнитивного агента: обучение структуре Qwen, фокус мышления, исправление устройств.")
    ...
    loaded_vocab = brain.load_knowledge("brain_knowledge_fixed.pth")
    ...
    try:
        while True:
            ...
            if conversation_count % 2 == 0:
                brain.save_knowledge("brain_knowledge_fixed.pth", vocab)
                print("💾 Сохранено!")
