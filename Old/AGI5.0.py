...
def clean_qwen_response(text: str) -> str:
    """Очищает ответ Qwen: удаляет markdown, эмодзи, дубли, оставляет только текст."""
    # Удаляем эмодзи и спецсимволы, кроме пунктуации
    ...
    # Удаляем лишние знаки в начале/конце
    text = re.sub(r'^[\*\.\!\?\:\-\–—\s]+', '', text)
    text = re.sub(r'[\*\.\!\?\:\-\–—\s]+$', '', text)
...
def classify_and_tag_response(text: str) -> str:
    ...
    # Удаляем дублирующие предложения
    sentences = []
    seen = set()
    for sent in re.split(r'(?<=[.!?])\s+', text):
        clean_sent = re.sub(r'[^\w\s]', '', sent.lower()).strip()
        if clean_sent and clean_sent not in seen and len(clean_sent) > 3:
...
def _detect_sentence_type(sentence: str) -> str:
    ...
    social_keywords = ["привет", "здравствуй", "добрый", "спасибо", "пожалуйста", "извини", "хорошо", "ладно", "ок",
                       "приветствую"]
    if any(kw in s for kw in social_keywords) or len(s.split()) <= 4:
        return "SOC"
...
class MetaCognition:
    ...
    def should_reflect(self, user_input: str, qwen_resp: str, brain_resp: str, vocab: Dict[str, int]) -> Tuple[
        bool, str]:
...
class CognitiveNetwork(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int = 256, hidden_size: int = 512, eos_token_id: int = 1):
        ...
        # Типы клеток
        self.type_to_stage = {
            "SOC": "social", "FCT": "fact", "CAU": "cause",
            "PRC": "procedure", "OPN": "opinion", "MET": "meta", "CRT": "creative"
        }
    ...
    def _process_step(self, token_emb: torch.Tensor, allowed_stages: Optional[Set[str]] = None):
    ...
    def process_sequence(self, input_tokens: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(seq_len):
            ...
            # Определяем разрешённые типы по входу
            allowed = self._get_allowed_stages_from_input(input_tokens[0, :t + 1].tolist())
            logits = self._process_step(token_emb, allowed)
            outputs.append(logits)

        return torch.stack(outputs, dim=1)

    def _get_allowed_stages_from_input(self, input_ids: List[int]) -> Set[str]:
        # Ищем последний маркер в входе
        for token_id in reversed(input_ids):
            if 3 <= token_id <= 9:  # ID маркеров
                marker_map = {3: "SOC", 4: "FCT", 5: "CAU", 6: "PRC", 7: "OPN", 8: "MET", 9: "CRT"}
                marker = marker_map.get(token_id, "FCT")
                stage = self.type_to_stage.get(marker, "fact")
                return {stage}
        return {"social", "fact"}  # по умолчанию
    ...
    List[int]:
        ...
        with torch.no_grad():
            ...
            for t in range(combined.size(1)):
                token_emb = combined[:, t, :]
                allowed = self._get_allowed_stages_from_input(input_tokens[0, :t + 1].tolist())
                _ = self._process_step(token_emb, allowed)
        ...
        # Определяем разрешённые типы для генерации
        allowed_stages = self._get_allowed_stages_from_input(input_tokens[0].tolist())
        ...
        for _ in range(max_length):
            with torch.no_grad():
                ...
                for _ in range(self.thought_cycles):
                    logits = self._process_step(token_emb, allowed_stages)
        ...
        if not generated:
            fallback = [10, 13, 33, 39]  # привет, день, хорошо, .
    ...
    def _check_cell_creation(self, user_input: str, qwen_resp: str, brain_resp: str):
        ...
        if len(high_activation) >= 3 and random.random() < 0.2:
            cell_types = ["social", "fact", "cause", "procedure", "opinion", "meta", "creative"]
            new_type = random.choice(cell_types)
            new_id = self.add_cell(f"{new_type}_adaptive", self.hidden_size, self.hidden_size, self.hidden_size,
                                   new_type)
    ...
    def load_knowledge(self, filepath: str):
        ...
        try:
            ...
            print(f"🧠 Загружено {len(self.cells)} клеток, словарь: {len(saved_vocab)} слов")
            return saved_vocab
    ...
    def expand_vocabulary(self, new_vocab_size: int):
        """Динамически расширяет embedding и final_proj."""
        if new_vocab_size <= self.vocab_size:
            return

        print(f"🔄 Расширение словаря с {self.vocab_size} до {new_vocab_size}")

        # Обновляем embedding
        old_embedding = self.embedding
        new_embedding = nn.Embedding(new_vocab_size, self.embedding_dim, padding_idx=0)
        new_embedding.weight.data[:self.vocab_size] = old_embedding.weight.data
        nn.init.normal_(new_embedding.weight.data[self.vocab_size:], mean=0.0, std=0.02)
        self.embedding = new_embedding

        # Обновляем final_proj
        old_proj = self.final_proj
        new_proj = nn.Linear(self.hidden_size, new_vocab_size)
        new_proj.weight.data[:self.vocab_size] = old_proj.weight.data
        new_proj.bias.data[:self.vocab_size] = old_proj.bias.data
        nn.init.normal_(new_proj.weight.data[self.vocab_size:], mean=0.0, std=0.02)
        nn.init.zeros_(new_proj.bias.data[self.vocab_size:])
        self.final_proj = new_proj
...
class BrainTeacher:
    ...
    def teach_brain(self, brain: CognitiveNetwork, user_input: str, vocab: Dict[str, int], ivocab: Dict[int, str]):
        ...
        # Динамически расширяем словарь модели
        current_vocab_size = len(vocab) + 10000
        if current_vocab_size > brain.vocab_size:
            brain.expand_vocabulary(current_vocab_size)
            # Обновляем ivocab
            ivocab.update({v: k for k, v in vocab.items() if v not in ivocab})
...
def main():
    ...
    if loaded_vocab is not None:
        ...
        brain.vocab_size = len(vocab) + 10000
        brain.expand_vocabulary(brain.vocab_size)
    ...
    reflection_buffer = []
    MAX_REFLECTION_CHAIN = 3  # уменьшено
    reflection_chain = 0
    seen_questions = set()
    ...
    while True:
        try:
            if reflection_buffer:
                ...
                if user_input in seen_questions:
                    continue
                seen_questions.add(user_input)
