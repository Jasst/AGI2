...
- динамической генерацией вопросов,
- bridge-клетками при семантическом разрыве.
"""
import os
import re
import math
...
class CognitiveNetwork(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int = 256, hidden_size: int = 512, eos_token_id: int = 1, device: Optional[torch.device] = None):
        ...
        self.inner_monologue: List[str] = []  # 🧠 Внутренний монолог
        self.to(self.device)
    ...
    def _generate_inner_thought(self, stage: str, activation: float) -> str:
        prompts = {
            "fact": "Я определяю суть: ",
            "cause": "Причина, вероятно, в том, что ",
            "procedure": "Как это сделать? ",
            "opinion": "Мне кажется, что ",
            "meta": "Стоит задуматься: ",
            "creative": "Можно представить это как ",
            "social": "Общаюсь: ",
        }
        return prompts.get(stage, "Я размышляю: ") + f"(активность {activation:.2f})"
    ...
    def _adaptive_thought_cycles(self, token_emb: torch.Tensor, max_cycles: int = 6, delta_threshold: float = 0.02):
        ...
        x = token_emb
        thought_stages = ["social", "fact", "cause", "procedure", "opinion", "meta", "creative"]
        prev_act = None

        for cycle in range(max_cycles):
            current_context = x

            for stage in thought_stages:
                stage_cells = [cid for cid in self.cells if stage in self.cells[cid].cell_type]
                if not stage_cells:
                    continue

                # передача состояния от топ-активных клеток предыдущих циклов
                external_state = None
                if cycle > 0:
                ...
                for cid in stage_cells:
                    ...
                    cell_output, new_hx, new_cx, act = cell(current_context, hx, cx, external_state)
                    self.cell_states[cid] = (new_hx, new_cx)
                    self.cell_activations[cid] = act
                    stage_outputs.append(cell_output)

                    # логируем внутреннюю мысль
                    if act > 0.2:
                        self.inner_monologue.append(self._generate_inner_thought(stage, act))

                if stage_outputs:
                    current_context = torch.mean(torch.stack(stage_outputs), dim=0)
    ...
    def _check_cell_creation(self, user_input: str, qwen_resp: str, brain_resp: str):
        ...
        # Bridge-клетки при семантическом разрыве
        similarity = self.meta_cog._estimate_similarity_semantic(qwen_resp, brain_resp)
        if similarity < 0.25 and random.random() < 0.4:
            topic_hash = safe_cell_name(user_input[:20].replace(" ", "_"))
            self.add_cell(f"bridge_{topic_hash}", self.hidden_size, self.hidden_size, "meta")
            print(f"🧬 Создана bridge-клетка для разрешения противоречия: {topic_hash}")
    ...
    def _formulate_deep_question(self, user_input: str, qwen_resp: str, brain_resp: str, reason: str, vocab: Dict[str, int]) -> str:
        # Получаем доминирующий тип мышления
    ...
    def get_thought_trace(self) -> str:
        lines = ["🧠 Трассировка мышления:"]
        for thought in self.inner_monologue:
            lines.append(f"  • {thought}")
        if len(lines) == 1:
            lines.append("  (нет активных мыслей)")
        return "\n".join(lines)
...
def main():
    print("🧠 Запуск улучшенного когнитивного агента v2: стадийное мышление, внутренний монолог, bridge-клетки.")
