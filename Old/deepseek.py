...
Улучшенная когнитивная сеть с приоритизированным replay, mixed precision,
упаковкой батчей, внешними эмбеддингами (SentenceTransformers если доступен)
...
class PrioritizedReplay:
    ...
    def add(self, inp: List[int], target: List[int], meta: Optional[Dict] = None, priority: Optional[float] = None):
        ...
        if priority is None:
            # initial large priority to be sampled soon
...
# ======================
# META-COGNITION WITH EXTERNAL EMBEDDINGS
# ======================
    ...
    def _embed_text_external(self, text: str) -> np.ndarray:
        if self.external_model is not None:
            try:
                ...
                pass
        # fallback to network embeddings if available
        ...
        # final fallback: simple bag of words vector
...
class CognitiveNetwork(nn.Module):
    ...
    def _process_step(self, token_emb: torch.Tensor):
        ...
        for stage in stages:
            ...
            if stage_outputs:
                stacked = torch.stack(stage_outputs, dim=0)
    ...
    def process_sequence(self, input_tokens: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if attention_mask is None:
            ...
            # construct key_padding_mask for MultiheadAttention (bool mask where True=pad)
    ...
    def expand_vocab(self, new_vocab_size: int):
        ...
        # set first old_vocab columns from old_out transpose
        with torch.no_grad():
...
# ======================
# УЧИТЕЛЬ (LM Studio / Qwen) с батчевой упаковкой и mixed precision
# ======================
...
class BrainTeacher:
    ...
    def teach_brain(self, brain: CognitiveNetwork, user_input: str, vocab: Dict[str, int], ivocab: Dict[int, str],
                    epochs: int = 2, batch_size: int = 8, teacher_forcing_start: float = 1.0):
        ...
        # Add to prioritized replay
        brain.replay.add(input_seq, target_seq, meta={"qwen": tagged_qwen, "raw_qwen": raw_qwen}, priority=1.0)

        # Build training batch: current example + samples from replay
        # Use replay's prioritized sampling
        ...
        # Sort by length descending to help packing
        ...
        for ep in range(epochs):
            teacher_forcing_ratio = max(0.05, teacher_forcing_start * (1.0 - 0.03 * ep))
            optimizer.zero_grad()
            ...
            if scaler is not None:
                with torch.cuda.amp.autocast():
                    ...
                    losses = losses * mask
                    # compute importance-weighted per-sequence loss
            ...
            # Update priorities for replay based on per-sample loss
            per_sample_losses = seq_losses.detach().cpu().numpy().tolist()
            # sample_idxs correspond to each row (None means current example)
            updated_priorities = []
            update_idxs = []
        ...
        brain.eval()
        with torch.no_grad():
            brain_tokens = brain.generate_sequence(torch.tensor([input_seq], dtype=torch.long, device=brain.device),
...
def main():
    print("🧠 Запуск когнитивного агента: приоритетный replay, packed batches, внешние эмбеддинги.")
