...
# -*- coding: utf-8 -*-
"""
🧠 САМООБУЧАЮЩИЙСЯ МИНИ-МОЗГ БОТ v9.0 - С ПОДДЕРЖКОЙ ФАЙЛОВОЙ СИСТЕМЫ
✅ РАБОЧАЯ краткосрочная и долгосрочная память
✅ Веб-поиск с валидацией
✅ 🔥 СОХРАНЕНИЕ файлов: txt, json, py
✅ 🔥 ЧТЕНИЕ файлов из личной директории пользователя
...
from pathlib import Path  # 🔥 ФАЙЛОВАЯ СИСТЕМА: обязательный импорт
...
USER_FILES_DIR = "user_files"  # 🔥 ФАЙЛОВАЯ СИСТЕМА: основная директория файлов
CACHE_DIR = "cache"
...
# ==================== 🔥 ЯДРО СОХРАНЕНИЯ ФАЙЛОВ ====================
class FileSaveCore:
    """Ядро для физического сохранения файлов на диск"""
    ...
    async def can_handle(self, query: str) -> Tuple[bool, float]:
        """Определение запроса на сохранение файла"""
        q = query.lower()
        save_words = ['сохрани', 'запиши', 'сохранить', 'записать', 'в файл']
        format_words = list(self.formats.keys()) + ['.txt', '.json', '.py']

        has_save = any(w in q for w in save_words)
        has_format = any(w in q for w in format_words)
        is_question = '?' in q or any(x in q for x in ['как', 'умеешь', 'можешь', 'научи'])

        if has_save and has_format and not is_question:
            return True, 0.92
        return False, 0.0

    async def execute(self, query: str, user_id: str, last_response: str = "") -> Dict[str, Any]:
        """ФИЗИЧЕСКОЕ СОХРАНЕНИЕ ФАЙЛА НА ДИСК"""
        # 1. Определяем формат
        fmt = '.txt'
        for word, ext in self.formats.items():
        ...
        filename_match = re.search(
            r'(?:сохрани|запиши|сохранить|записать)\s+([^\s]+?)(?:\s+в)?\s+(?:файл\s+)?(?:как\s+)?',
            query,
            re.IGNORECASE
        )
        if filename_match:
            ...
            # Генерируем имя
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"save_{timestamp}{fmt}"

        # 3. Санитизация
        filename = self._sanitize_filename(filename)

        # 4. Извлекаем содержимое
        # Сначала пробуем из запроса
        content_match = re.search(
            r'(?:сохрани|запиши)\s+[^\s]+\s+(?:в\s+)?(?:файл\s+)?(?:как\s+)?(?:в\s+)?(?:txt|json|py|текст|пайтон|питон)?\s*(.+)',
            query,
            re.IGNORECASE | re.DOTALL
        )
        content = content_match.group(1).strip() if content_match else ""

        # Если нет явного содержимого - используем последний ответ бота
        if not content and last_response:
            content = re.sub(r'[*_`~#]', '', last_response)
            content = re.sub(r'\n{3,}', '\n\n', content)

        # Если совсем пусто - тестовое содержимое
        if not content.strip():
            content = f"Сохранено по запросу: {query}\nВремя: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"

        # 5. Подготовка пути пользователя
        user_dir = self.base_dir / f"user_{user_id}"
        user_dir.mkdir(exist_ok=True)

        # 6. Уникальное имя (если файл существует)
        ...
        try:
            ...
            if fmt == '.json':
                ...
                # TXT и PY - записываем как есть
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(content)

            # 8. Подтверждение
            size_bytes = filepath.stat().st_size
            size_str = f"{size_bytes} байт" if size_bytes < 1024 else f"{size_bytes / 1024:.1f} КБ"

            return {
                'success': True,
                'answer': f'✅ **Файл сохранён**\n📁 `{filename}`\n📦 {size_str}\n💡 Сохранено в: `{user_dir.name}/`',
                'confidence': 0.95,
                'strategy': 'file_save',
                'metadata': {
...
# ==================== 🔥 ЯДРО ЧТЕНИЯ ФАЙЛОВ ====================
class FileReadCore:
    """Ядро для чтения файлов пользователя"""

    def __init__(self, base_dir: str = "user_files"):
        self.base_dir = Path(base_dir)
        self.base_dir.mkdir(exist_ok=True)
        self.max_file_size = 100 * 1024  # 100 КБ максимум для показа

    async def can_handle(self, query: str) -> Tuple[bool, float]:
        """Определение запроса на чтение файла"""
        q = query.lower()
        read_words = ['прочитай', 'открой', 'покажи', 'посмотри', 'файл', 'мои файлы', 'список файлов']
        has_read = any(w in q for w in read_words)
        is_question = '?' in q or any(x in q for x in ['как', 'умеешь', 'можешь'])

        if has_read and not is_question:
            return True, 0.90
        return False, 0.0

    def _get_user_files(self, user_id: str) -> List[Dict]:
        ...
        for ext in ['.txt', '.json', '.py']:
            for filepath in user_dir.glob(f'*{ext}'):
    ...
    async def execute(self, query: str, user_id: str) -> Dict[str, Any]:
        """Чтение файла или показ списка"""
        q = query.lower()
        user_dir = self.base_dir / f"user_{user_id}"

        # Случай 1: Показать список файлов
        if any(x in q for x in ['список файлов', 'мои файлы', 'покажи файлы', 'какие файлы']):
            files = self._get_user_files(user_id)

            if not files:
                return {
                    'success': True,
                    'answer': '📂 У вас пока нет сохранённых файлов.\nСохраните первый: `сохрани заметка.txt купить хлеб`',
                    'confidence': 0.9,
                    'strategy': 'file_read'
                }

            # Формируем список
            answer = f'📁 **Ваши файлы** (`{user_dir.name}/`):\n\n'
            total_size = 0

            for i, f in enumerate(files[:10], 1):  # Показываем максимум 10 файлов
                size_str = f"{f['size']} б" if f['size'] < 1024 else f"{f['size'] / 1024:.1f} КБ"
                answer += f"{i}. `{f['name']}` — {size_str} ({f['created']})\n"
                total_size += f['size']

            if len(files) > 10:
                answer += f"\n... и ещё {len(files) - 10} файлов\n"

            total_str = f"{total_size} б" if total_size < 1024 else f"{total_size / 1024:.1f} КБ"
        ...
        # Случай 2: Прочитать конкретный файл
        # Ищем имя файла в запросе
        filename_match = re.search(
            r'(?:прочитай|открой|покажи)\s+(?:файл\s+)?([^\s]+)',
            query,
            re.IGNORECASE
        )
        ...
        if not filename_match:
            files = self._get_user_files(user_id)
            if files:
                examples = ', '.join([f"`{f['name']}`" for f in files[:3]])
                return {
                    'success': True,
                    'answer': f'❓ Укажите имя файла.\nПримеры: {examples}\nИли покажите все: `покажи мои файлы`',
                    'confidence': 0.8,
                    'strategy': 'file_read'
                }
            else:
                return {
                    'success': True,
                    'answer': '📂 У вас нет файлов. Сохраните первый: `сохрани заметка.txt текст`',
                    'confidence': 0.9,
                    'strategy': 'file_read'
                }
        ...
        # Поиск файла (точное совпадение или частичное)
        files = self._get_user_files(user_id)
        target_file = None

        for f in files:
            if filename.lower() in f['name'].lower() or f['name'].lower().startswith(filename.lower()):
                target_file = f
                break

        if not target_file:
            examples = ', '.join([f"`{f['name']}`" for f in files[:3]]) if files else "нет файлов"
            return {
                'success': False,
                'answer': f'❌ Файл `{filename}` не найден.\nДоступные файлы: {examples}',
                'confidence': 0.85,
                'strategy': 'file_read'
            }
        ...
        # Чтение содержимого
        try:
            filepath = target_file['path']
            size_kb = target_file['size'] / 1024

            # Ограничение размера для показа
            if target_file['size'] > self.max_file_size:
                return {
                    'success': True,
                    'answer': f'📄 Файл `{filepath.name}` ({size_kb:.1f} КБ)\n⚠️ Слишком большой для показа (>100 КБ)\nРазмер: {target_file["size"]} байт',
                    'confidence': 0.9,
                    'strategy': 'file_read'
                }
            ...
            return {
                'success': True,
                'answer': f'📄 **{filepath.name}** ({size_kb:.1f} КБ)\n{content}',
                'confidence': 0.95,
                'strategy': 'file_read',
                'metadata': {'filename': filepath.name, 'size': target_file['size']}
            }

        except Exception as e:
            return {
                'success': False,
                'answer': f'❌ Ошибка чтения файла:\n`{str(e)[:150]}`',
                'confidence': 0.0,
                'strategy': 'file_read'
            }
...
class AutoMemorySystem:
    ...
    def __init__(self, user_id: str):
        ...
        self._last_bot_response = ""  # 🔥 ФАЙЛОВАЯ СИСТЕМА: храним последний ответ для сохранения
...
JSON:"""
    ...
    # 🔥 ФАЙЛОВАЯ СИСТЕМА: методы для работы с последним ответом
    def set_last_bot_response(self, response: str):
        """Сохранение последнего ответа бота для возможного сохранения"""
        self._last_bot_response = response[:5000]  # Ограничиваем размер

    def get_last_bot_response(self) -> str:
        """Получение последнего ответа бота"""
...
class MetaCognitiveManager:
    """
    🧠 МЕТА-КОГНИТИВНЫЙ МЕНЕДЖЕР v9.0
    - Автоматическая память
    - Валидированный веб-поиск
    - 🔥 Файловая система: сохранение и чтение
    ...
    def __init__(self, user_id: str, llm_caller: callable):
        ...
        # 🔥 ФАЙЛОВАЯ СИСТЕМА: инициализация ядер
        self.file_save_core = FileSaveCore()
        self.file_read_core = FileReadCore()
    ...
    async def process_query(self, query: str) -> Dict[str, Any]:
        ...
        # 4. 🔥 ФАЙЛОВАЯ СИСТЕМА: сохраняем ответ для возможного сохранения
        if result.get('success', False) and 'answer' in result:
            self.memory.set_last_bot_response(result['answer'])
        # 5. СОХРАНЕНИЕ ОТВЕТА В ПАМЯТЬ
        await self.memory.analyze_and_store_message('assistant', result['answer'])
        # 6. ПРИНУДИТЕЛЬНОЕ СОХРАНЕНИЕ (для надёжности)
    ...
    async def _select_strategy(self, query: str) -> Dict[str, Any]:
        ...
        # 3. 🔥 ФАЙЛОВАЯ СИСТЕМА: проверка чтения файлов (высокий приоритет)
        can_read, conf_read = await self.file_read_core.can_handle(query)
        if can_read and conf_read > 0.85:
            return {'name': 'file_read', 'confidence': conf_read}
        # 4. Если есть веб-приоритетные слова - СРАЗУ на веб-поиск
        ...
        # 5. ТОЛЬКО ПОТОМ проверяем datetime (если нет веб-приоритета)
        ...
        # 6. 🔥 ФАЙЛОВАЯ СИСТЕМА: проверка сохранения файлов
        ...
        # 7. Общая проверка веб-поиска (для других ключевых слов)
    ...
    async def _execute_strategy(self, query: str, strategy: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # === 🔥 ФАЙЛОВАЯ СИСТЕМА: ЧТЕНИЕ ФАЙЛОВ ===
            if strategy_name == 'file_read':
                result = await self.file_read_core.execute(query, self.user_id)
                # Сохраняем факт чтения в память
                if result['success'] and 'metadata' in result:
                    await self.memory.add_to_long_term(
                        content=f"Прочитан файл: {result['metadata']['filename']}",
            ...
            if strategy_name == 'file_save':
                last_response = self.memory.get_last_bot_response()
                result = await self.file_save_core.execute(query, self.user_id, last_response)
                # Сохраняем факт сохранения в память
                if result['success'] and 'metadata' in result:
                    await self.memory.add_to_long_term(
                        content=f"Сохранён файл: {Path(result['metadata']['filepath']).name}",
                        category='user_activity',
                        importance=0.6,
                        source='file_save'
                    )
...
6. 🔥 Пользователь может просить сохранять файлы (сохрани ...) или читать их (покажи файлы)
7. Будь полезным, точным и дружелюбным
ОТВЕТ:"""
        try:
...
class TelegramBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        file_status = "✅ Доступна"  # 🔥 ФАЙЛОВАЯ СИСТЕМА
        await update.message.reply_text(
            f"🧠 **МИНИ-МОЗГ БОТ v9.0**\n"
            f"\n"
            f"✨ **Новые возможности:**\n"
            f"• ✅ РАБОЧАЯ долгосрочная память с автосохранением\n"
            f"• 🌐 Веб-поиск с валидацией и синтезом: {web_status}\n"
            f"• 🔥 Файловая система: сохранение и чтение файлов: {file_status}\n"
    ...
    async def memory_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = (
            f"🧠 **СТАТИСТИКА ПАМЯТИ**\n"
            f"\n"
            f"📝 Краткосрочная память: {stats['short_term_count']} сообщений\n"
            ...
            f"\n"
            f"💬 **Ваш стиль:** {stats['communication_style']}\n"
        )
    ...
    async def help_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            "6️⃣ 🔥 Работает с файлами: сохранение и чтение\n"
            "\n"
            "**Примеры запросов:**\n"
            ...
            "• 🔥 `покажи мои файлы` - список сохранённых файлов\n"
            "• 🔥 `прочитай заметка.txt` - чтение файла\n"
            "\n"
            "**Команды:**\n"
...
async def main_async():
    ...
    print("\n" + "=" * 70)
    print("🚀 МИНИ-МОЗГ БОТ v9.0 - С ПОДДЕРЖКОЙ ФАЙЛОВОЙ СИСТЕМЫ")
    print("=" * 70)
    print(f"⏰ Запуск: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
    print(f"🧠 Память: {MEMORY_DIR}/")
    print(f"📁 Файлы: {USER_FILES_DIR}/")
    ...
    print("   • 🔥 Файловая система: сохранение и чтение")
