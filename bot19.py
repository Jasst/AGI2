...
# -*- coding: utf-8 -*-
"""
🧠 HYBRID NEURAL BRAIN v21.0 - COGNITIVE UPGRADE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Новое в v21 (когнитивные улучшения):

  🎯 Умная L1-память (Smart Working Memory):
       • Динамический буфер: удерживает не последние N, а
         семантически самые связанные с текущим запросом сообщения
       • Мини-RAG по краткосрочной истории (BM25 по L1)
       • Всегда включает последние 3 сообщения + 3 релевантных

  🔗 Глубокие причинные цепочки (Deep Causal Reasoning):
       • Обратное цепочечное рассуждение: "почему X?" →
         строит полное дерево причин и передаёт в промпт
       • Прямое: "что будет если X?" → дерево следствий
       • Контрфактуальное: "что если не X?" → альтернативные пути
       • Структурированный контекст в промпте, а не просто строка

  💭 Эмоциональный RAG (Emotion-Aware Retrieval):
       • При негативной эмоции вытаскивает эпизоды с похожим
         состоянием + как они разрешились → паттерны выхода
       • Emotion_boost в скоринге RAGv2

  🗑️ Активное забывание (Active Forgetting):
       • Нейроны без обращений >30 дней → архивный режим
       • Архивные не участвуют в распространении активации
       • Могут быть реактивированы при релевантном запросе

  🔄 Двухуровневая рефлексия (Two-Level Reflection):
       • Быстрая (каждые 10 сообщений): ищет противоречия в сессии
       • Глубокая (каждые 50 / при старте): обновляет L3 из реального поведения

  📈 Фидбек-петля температуры (Temperature Feedback Loop):
       • Если пользователь переспрашивает — снижаем temp для темы
       • Если доволен — повышаем чуть
       • История качества ответов per cluster

  🔮 Марковские цепи 2-го порядка (2nd-Order Topic Prediction):
       • Переход учитывает предыдущие две темы
       • Точнее предсказывает следующую тему разговора

Сохранено из v20: стохастический дрейф, BM25 RAG,
...
TONIC_MAX              = 0.25
AUTOSAVE_EVERY         = 10
ARCHIVE_AFTER_DAYS     = 30      # Нейроны без обращений → архив
ARCHIVE_ACTIVATION_CAP = 0.05   # Максимальная активация архивных нейронов
...
RAG_BM25_K1    = 1.5
RAG_BM25_B     = 0.75
RAG_EMOTION_BOOST = 0.25        # Бонус за совпадение эмоции

# L1 Smart Memory
L1_RECENT_COUNT   = 3           # Всегда включаем последние N сообщений
L1_RELEVANT_COUNT = 3           # + N семантически релевантных из истории
L1_MAX_HISTORY    = 30          # Максимальный размер буфера L1
...
TEMP_CREATIVE   = 0.90
TEMP_DEFAULT    = 0.65
TEMP_FEEDBACK_STEP = 0.05       # Шаг коррекции температуры по фидбеку

# Рефлексия
REFLECTION_FAST_EVERY  = 10     # Быстрая рефлексия
REFLECTION_DEEP_EVERY  = 50     # Глубокая рефлексия
...
class ActivationDecay:
    ...
    def is_archive_candidate(last_iso: str) -> bool:
        """Нейрон не активировался дольше ARCHIVE_AFTER_DAYS дней"""
        try:
...
# ══════════════════════════════════════════
# АДАПТИВНАЯ ТЕМПЕРАТУРА + ФИДБЕК-ПЕТЛЯ (v21)
# ══════════════════════════════════════════
class TemperatureAdapter:
    """
    🌡️ Адаптивная температура с фидбек-петлёй.

    Новое в v21: история качества ответов per-cluster.
    Если пользователь переспрашивает (признаки непонимания) →
    снижаем температуру для данного кластера.
    Если благодарит или принимает ответ без переспросов → повышаем.
    """
    CLUSTER_TEMPS = {
    ...
    # Признаки непонимания / переспроса
    CONFUSION_SIGNALS = [
    ...
    def feedback(self, text: str, cluster: Optional[str]):
        """Обновить температурный профиль по тексту пользователя"""
        if not cluster:
            return
        tl = text.lower()
...
# ══════════════════════════════════════════
# УМНАЯ L1-ПАМЯТЬ (v21 — Smart Working Memory)
# ══════════════════════════════════════════
class SmartWorkingMemory:
    """
    🎯 L1 — умная рабочая память.

    Вместо "последние 6 сообщений" использует гибридный подход:
    - Всегда: последние L1_RECENT_COUNT сообщений (контекст диалога)
    - Плюс: L1_RELEVANT_COUNT семантически близких к текущему запросу

    Это позволяет "вспоминать" важные детали из начала разговора,
    даже если между ними и текущим сообщением много других тем.
    """
    ...
    def get_context(self, query: str) -> List[Dict]:
        """Возвращает релевантный контекст для данного запроса"""
        if not self.history:
            return []
        ...
        # Скорим все остальные сообщения по семантическому перекрытию
        candidates = []
        ...
        # Объединяем: релевантные + последние, сохраняя порядок
...
# ══════════════════════════════════════════
# RAG v2 — с поддержкой эмоционального буста (v21)
# ══════════════════════════════════════════
@dataclass
class RAGResult:
    content:      str
...
class RAGEngineV2:
    """
    🔍 RAG с BM25 + эмоциональный буст (v21).

    Новое: emotion_boost — если текущее настроение пользователя
    совпадает с эмоцией эпизода, он получает бонус к скору.
    При негативном настроении это позволяет вытаскивать эпизоды
    "как я выходил из похожей ситуации раньше".
    """
...
# ══════════════════════════════════════════
# ТРЁХУРОВНЕВАЯ ПАМЯТЬ — L3 семантическая
# ══════════════════════════════════════════
class SemanticMemoryL3:
    ...
    def force_rebuild(self):
        """Принудительный пересчёт (для глубокой рефлексии)"""
        self._rebuild_facts()
...
# ══════════════════════════════════════════
# ПРЕДСКАЗАНИЕ ТЕМАТИКИ — МАРКОВ 2-го ПОРЯДКА (v21)
# ══════════════════════════════════════════
class TopicPredictor:
    """
    🔮 Марковские цепи 2-го порядка.

    Вместо P(next | current) вычисляем P(next | prev, current).
    Если нет данных для 2-го порядка — откатываемся к 1-му.

    Пример: после [технологии → работа] чаще следует [деньги],
    а после [общение → работа] — [учёба]. Первый порядок это не различит.
    """

    def __init__(self, save_path: str):
        self.save_path   = save_path
        self.transitions: Dict[str, Dict[str, int]] = {}    # 1-й порядок
        self.transitions2: Dict[str, Dict[str, int]] = {}   # 2-й порядок
    ...
    def get_hint(self, cluster: str, prev_cluster: str = None) -> str:
        ...
        if prob > 0.4: return f"Вероятно, следующий вопрос о «{best}» ({prob:.0%}){' '+order if order else ''}."
        return ""


# ══════════════════════════════════════════
# ЛОГИЧЕСКИЕ ЦЕПОЧКИ — ГЛУБОКОЕ РАССУЖДЕНИЕ (v21)
# ══════════════════════════════════════════
@dataclass
class CausalLink:
...
class LogicalChainEngine:
    """
    🔗 Глубокие причинные цепочки (v21).

    Новое:
    - backward_chain: "почему X?" → дерево причин с глубиной
    - forward_chain: "что будет если X?" → дерево следствий
    - counterfactual: "что если не X?" → альтернативные пути
    - structured_context: возвращает структурированный блок для промпта
    """
    ...
    def backward_chain(self, effect: str, depth: int = 3) -> Dict[str, Any]:
        """
        Дерево причин для данного следствия.
        Возвращает: {'node': effect, 'causes': [{'node': cause, 'strength': x, 'causes': [...]}]}
        """
        def _recurse(node: str, visited: Set[str], d: int) -> Dict:
            ...
            return {'node': node, 'causes': causes[:3]}  # max 3 причины на уровень

        return _recurse(effect, {effect}, depth)

    def forward_chain(self, cause: str, depth: int = 3) -> Dict[str, Any]:
        """Дерево следствий для данной причины."""
        def _recurse(node: str, visited: Set[str], d: int) -> Dict:
            effects = []
            if d > 0:
                for l in sorted(self.links.values(), key=lambda x: -x.strength):
    ...
    def counterfactual(self, concept: str) -> List[str]:
        """
        Что если 'concept' отсутствует?
        Ищем всё, что зависит от concept (следствия с positive-связью).
        """
        dependent = [
    ...
    def format_causal_for_prompt(self, keyword: str, query: str) -> str:
        """
        Строит структурированный контекст причинных связей для промпта.
        Определяет тип вопроса и выбирает нужный вид цепочки.
        """
        tl = query.lower()
        lines = []
        ...
        if any(w in tl for w in ['почему', 'причина', 'из-за чего', 'отчего']):
            ...
            # Показываем цепочку по умолчанию
...
# ══════════════════════════════════════════
# НЕЙРО-КОРА — с активным забыванием (v21)
# ══════════════════════════════════════════
@dataclass
class NeuroNode:
    ...
    archived: bool = False  # v21: флаг архивного нейрона
...
class DynamicNeuralCortex:
    ...
    def _reactivate_if_archived(self, node: NeuroNode):
        """Реактивировать архивный нейрон при обращении"""
        if node.archived:
            node.archived = False
    ...
    def process_input(self, text: str, now: datetime = None) -> Tuple[List[str], List[ChronoNode], Optional[str], Optional[str]]:
        ...
        for i, kw1 in enumerate(keywords):
            ...
            self._reactivate_if_archived(n1)   # v21: реактивация при обращении
        ...
        for label in activated_labels:
            ...
            for key, syn in list(self.synapses.items()):
                if syn.source == nid and syn.weight > 0.4:
                    ...
                    if target and not target.archived:  # v21: архивные не участвуют в распространении
        ...
        # Затухание + проверка на архивирование
        for node in self.nodes.values():
            ...
            if node.archived:
                ...
                # Архивируем если долго не использовался и малый опыт
...
# ══════════════════════════════════════════
# ДВУХУРОВНЕВАЯ РЕФЛЕКСИЯ (v21)
# ══════════════════════════════════════════
class ReflectionEngine:
    """
    🔄 Двухуровневая рефлексия (v21).

    Быстрая (каждые REFLECTION_FAST_EVERY сообщений):
    - Ищет противоречия в текущей сессии
    - Краткие наблюдения о паттернах

    Глубокая (каждые REFLECTION_DEEP_EVERY сообщений):
    - Сравнивает реальное поведение с L3
    - Принудительно обновляет L3 из накопленной статистики
    - Добавляет долгосрочные инсайты
    """
    ...
    def tick(self) -> Tuple[bool, bool]:
        """Возвращает (fast_trigger, deep_trigger)"""
        self._counter += 1
    ...
    async def reflect_deep(self, memory: 'HybridMemorySystem') -> Optional[str]:
        """Глубокая рефлексия: обновляет L3, строит долгосрочный профиль"""
        # Принудительно пересчитываем L3
...
class HybridMemorySystem:
    def __init__(self, user_id: str):
        ...
        # Компоненты
        self.l1        = SmartWorkingMemory(os.path.join(self.dir, "short_term.json"))  # v21
        ...
        self.predictor = TopicPredictor(os.path.join(self.dir, "topic_transitions.json"))  # v21: 2-й порядок
        self.semantic_l3 = SemanticMemoryL3(os.path.join(self.dir, "semantic_l3.json"))
        self.temp_adapter = TemperatureAdapter(os.path.join(self.dir, "temp_feedback.json"))  # v21

        self.now = datetime.now()
        self._msg_count    = 0
        self._prev_cluster: Optional[str] = None
        self._pprev_cluster: Optional[str] = None  # v21: для предсказания 2-го порядка
    ...
    async def process(self, text: str):
        ...
        # 3. Фидбек температуры (v21): анализируем предыдущий ответ
        if self._prev_cluster:
            self.temp_adapter.feedback(text, self._prev_cluster)

        # 4. Переход тем (2-й порядок)
        ...
        # 10. RAG v2 с эмоциональным бустом (v21)
        ...
        # 11. Причинный контекст (v21: структурированное глубокое рассуждение)
        causal_ctx = ""
        ...
        # 13. Адаптивная температура с фидбеком (v21)
        temp = self.temp_adapter.get(cluster_name, session_emotion, text)

        # 14. Предсказание тем 2-го порядка (v21)
        ...
        # Обновляем L1 (v21: SmartWorkingMemory)
    ...
    def handle_logic_query(self, text: str) -> Optional[str]:
        """v21: расширенная обработка с деревьями причин и следствий"""
    ...
    def save_all(self):
        ...
        self.l1.save(); self.temp_adapter.save()
...
class HybridBot:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Логический запрос (v21: глубокий)
    ...
    async def cmd_l1(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Показать умную L1-выборку для последнего сообщения"""
        uid   = str(update.effective_user.id); brain = self.get_brain(uid)
    ...
    async def cmd_temp(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Показать температурные профили по кластерам"""
        uid   = str(update.effective_user.id); brain = self.get_brain(uid)
    ...
    async def cmd_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            ...
            "/wipe            — очистка памяти\n\n"
            "💡 Новое в v21:\n"
            "• L1 выбирает релевантные, а не просто последние\n"
            "• /chain показывает дерево причин И следствий\n"
            "• Архивные нейроны: забытые темы засыпают\n"
            "• Рефлексия: быстрая каждые 10, глубокая каждые 50\n"
            "• Температура учится на ваших реакциях\n"
            "• Предсказание тем учитывает 2 предыдущие темы"
        )
...
# ══════════════════════════════════════════
async def main():
    print("🚀 COGNITIVE UPGRADE v21.0 STARTING...")
    ...
    for cmd, handler in [
        ...
        ("l1", bot.cmd_l1), ("temp", bot.cmd_temp),
    ...
    try:
        ...
        print("✅ COGNITIVE UPGRADE v21 ГОТОВ 🧠🎯")
        print("💡 Новые команды: /l1, /temp, /chain (с деревьями)")
